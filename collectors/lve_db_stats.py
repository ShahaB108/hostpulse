...
def collect(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        query = """
            SELECT
                uid,
                ROUND(MAX(sum_cpu), 2) AS max_cpu,
                ROUND(AVG(sum_cpu), 2) AS avg_cpu,
        ...
        for row in rows:
            uid, max_cpu, avg_cpu, total_read, total_write, samples = row
...
if __name__ == "__main__":
    result = collect({
        "lve_db_path": "/var/lve/lvestats2.db",
        "ignored_users": set(),
    })
